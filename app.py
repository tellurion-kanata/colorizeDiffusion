...
def init_interface(opt, *args, **kwargs) -> None:
    ...
    img_block = partial(gr.Image, type="pil", height=300, interactive=True, show_label=True)
    with gr.Blocks(
        ...
        with gr.Row(elem_id="content-row", equal_height=False, variant="panel"):
                ...
                with gr.Accordion("🔧 Advanced Settings", open=True):
                    with gr.Row():
                        ...
                        latent_inpaint = gr.Checkbox(
                            label="Use latent inpainting (BG input required)",
                            value=False,
                            scale=2
                        )
                    with gr.Row():
            ...
            with gr.Column():
                result_gallery = gr.Gallery(
                    label='Output', show_label=False, elem_id="gallery", preview=True, type="pil"
                )
                ...
                with gr.Row():
                    ...
                    width = gr.Slider(label="Width", minimum=512, maximum=1536, value=768, step=64, scale=2)
                with gr.Row():
                    step = gr.Slider(label="Step", minimum=1, maximum=100, value=20, step=1, scale=1)
                    height = gr.Slider(label="Height", minimum=512, maximum=1536, value=768, step=64, scale=2)
                # with gr.Row():
