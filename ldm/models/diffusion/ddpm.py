...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if cond_key != self.first_stage_key:
                if cond_key == 'colorize':
                    xc = self.cond_stage_model.get_input(batch, device=self.device)
                elif cond_key in ['caption', 'coordinates_bbox', "txt", "reference"]:
                    xc = batch[cond_key]
    ...
    def shared_step(self, batch, **kwargs):
        xc, _ = self.get_input(batch, self.first_stage_key, cond_key=self.cond_stage_key)
        x, c = xc
        loss = self(x, c)
        return loss
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        unconditional_conditioning=None, unconditional_guidance_scale=1., return_x0=False,
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=0., return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            if self.cond_stage_key == "colorize":
                uc = self.cond_stage_model.get_unconditional_conditioning(c, unconditional_guidance_label)
            else:
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list=None, c_crossattn: list=None, c_adm=None):
        ...
        elif self.conditioning_key == 'concat-adm':
            assert c_adm is not None
            c_concat, c_adm = map(lambda t: torch.cat(t, 1), (c_concat, c_adm))
