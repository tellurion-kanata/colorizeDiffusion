...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # use_channel_attention=False,      # custom channel-wise attention in encoder and decoder
...
class MappingUnetModel(DualCondUnetModel):
    def __init__(self, context_dim, **kwargs):
        super().__init__(context_dim, **kwargs)
        dims = []
        self.mapper_modules = nn.ModuleList(
            [MemoryEfficientCrossAttention(query_dim=dims[i], heads=dims[i]//self.num_head_channels,
                                           dim_head=self.num_head_channels, context_dim=context_dim)
            for i in range(len(dims))]
        )

    def mapper_parameters(self):
        return self.mapper_modules.parameters()

    def forward(self, x, timesteps=None, concat=None, context=None, y=None, return_feat=False, **kwargs):
        assert (y is not None) == (
                self.num_classes is not None
        ), "must specify y if and only if the model is class-conditional"
        hs = []
