...
class HybridSketchEncoder(nn.Module):
    def __init__(self, in_ch, model_channels, ch_mults, dropout=0.):
        super().__init__()
        # sequential downsampler for precise sketch
        self.conv_downsampler = sequential_downsample(in_ch, model_channels)

        # One-step downsampler for rough sketch
        self.tokenizer = nn.Sequential(conv_nd(2, in_ch, 8, stride=8), nn.SiLU())
